...
-- Nome do aluno: Rosiane Muniz da Rocha
...
SELECT  NumeroPedido  AS 'Número do Pedido',
		CodigoProduto AS 'Codigo do Produto',
		Quantidade    AS 'Quantidade dos itens'
FROM ITEM_DE_PEDIDO
WHERE Quantidade = '35';    
GO
...
SELECT  P.NumeroPedido          AS 'Número do Pedido',
		COUNT (DISTINCT R.UnidadeProduto) AS 'Quantidade total de produtos',
		SUM (I.Quantidade)     AS 'Quantidade total de itens'		
	FROM PEDIDOS P INNER JOIN ITEM_DE_PEDIDO I
			ON P.NumeroPedido = I.NumeroPedido
		INNER JOIN	PRODUTOS R 
			ON R.CodigoProduto = I.CodigoProduto
...
SELECT  C.CodigoCliente    AS 'Código do Cliente',
		...
		R.DescricaoProduto AS 'Produto Comprado',
		R.ValorUnitario    AS 'Valor Unitário',
		I.Quantidade	   AS 'Quantidade',
		(R.ValorUnitario * I.Quantidade) AS 'Valor total do Pedido',
		P.PrazoEntrega     AS 'Prazo de Entrega',
		V.NomeVendedor     AS 'Nome do Vendedor'
FROM CLIENTES C INNER JOIN PEDIDOS P
	ON C.CodigoCliente = C.CodigoCliente
	INNER JOIN VENDEDORES V
	ON P.CodigoVendedor = V.CodigoVendedor 
	INNER JOIN ITEM_DE_PEDIDO I
	ON P.NumeroPedido = I.NumeroPedido
	INNER JOIN PRODUTOS R
	ON R.CodigoProduto = I.CodigoProduto
